...
function canQueenCaptureKing(queen, king) {
  const checkXLine = Math.abs(queen.x - king.x);
  const checkYLine = Math.abs(queen.y - king.y);

  return checkXLine === 0 || checkYLine === 0 || checkXLine === checkYLine;
}
...
function sortByAsc(arr) {
  ...
  function partition(l, r) {
    ...
    while (left <= right) {
      while (copyArr[left] < pivot) {
        left += 1;
      }
      while (copyArr[right] > pivot) {
        right -= 1;
      }
      if (left <= right) {
        [copyArr[left], copyArr[right]] = [copyArr[right], copyArr[left]];
        left += 1;
        right -= 1;
      }
    }
